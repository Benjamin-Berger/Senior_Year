...
bool Sudoku::FindUnassignedLocation(vector<vector<int> > &puzzle, int &row, int &col)
{
    for (row = 0; row < N; row++)
        for (col = 0; col < N; col++)
            if (puzzle[row][col] == 0)
...
/* Returns whether any assigned entry within the specified 3x3 box matches 
   the given number. */
bool Sudoku::UsedInBox(vector<vector<int> > &puzzle, int boxStartRow, int boxStartCol, int num)
{
