...
// void createBlank(Mat modified_image){
//     for(int i = 0; i < 9; i++){
//       for(int j = 0; j < 9; j++){
//         for(int x = 0; x < 55; x++){
//           for(int y = 0; y < 55; y++){
//             modified_image.at<uchar>(15 + (i* 71) + y, 10 + (j* 75) + x) = 255;
//           }
//         }
//       }
//     }

//     imwrite("TEST.bmp", modified_image);

// }

void Sudoku::createPuzzle(Mat modified_image, vector<vector<int> > &puzzle){
  // createBlank(modified_image);
...
Mat Sudoku::fillPuzzle(vector<vector<int> > &puzzle){
  Mat fin = imread("Fill/GRID.bmp", CV_LOAD_IMAGE_GRAYSCALE);
k  string place;
  for(int i = 0; i < 9; i++){
    for(int j = 0; j < 9; j++){
      ...
      for(int y = 0; y < 55; y++){
        for(int x = 0; x < 55; x++){
          fin.at<uchar>(15 + (i* 71) + y, 10 + (j* 75) + x) = temp.at<uchar>(y, x);
        }
      }
      temp.release();
    }
