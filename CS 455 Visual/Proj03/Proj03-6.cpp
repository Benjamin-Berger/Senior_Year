...
void IDCT(vector<vector<float> > hold8x8, vector<vector<float> > &DCT8x8){
...
void DCT1(vector<vector<float> > hold8x8, vector<vector<float> > &DCT8x8){
  int rows = hold8x8.size();
  int cols = hold8x8[0].size();
  vector<float> hold8;
...
void DCT9(vector<vector<float> > hold8x8, vector<vector<float> > &DCT8x8){
  int rows = hold8x8.size();
  int cols = hold8x8[0].size();
  vector<float> hold8;
...
int main(int argc, char **argv) {
  ...
  Mat modified_image1(original_image.rows, original_image.cols, CV_32FC3);
  Mat modified_image2(original_image.rows, original_image.cols, CV_32FC3);
  Mat modified_image3(original_image.rows, original_image.cols, CV_32FC3);
...
//code starts here
  ...
  for(int y = 0; y < original_image.rows; y++){
    for(int x = 0; x < original_image.cols; x++){
      ...
      modified_image1.at<cv::Vec3f>(y,x)[2] = i;
      //modified_image1.at<cv::Vec3f>(y,x)[1] = s;
      //modified_image1.at<cv::Vec3f>(y,x)[0] = h;
    }
  }
  ...
  for(int y = 0; y < original_image.rows; y += 8){
    for(int x = 0; x < original_image.cols; x += 8){
      ...
      get8x8(x, y, modified_image, hold8x8);


      IDCT(hold8x8, DCT8x8);

      stitch(x, y, DCT8x8, modified_image1);
  }
}
  ...
  for(int y = 0; y < original_image.rows; y += 8){
    for(int x = 0; x < original_image.cols; x += 8){
      ...
      stitch(x, y, DCT8x8, modified_image2);
  ...
  for(int y = 0; y < original_image.rows; y += 8){
    for(int x = 0; x < original_image.cols; x += 8){
      ...
      get8x8(x, y, modified_image2, hold8x8);


      IDCT(hold8x8, DCT8x8);

      stitch(x, y, DCT8x8, modified_image3);
  }
}
    ...
    switch(cvWaitKey(15)) {
      ...
      case 32:  //Swap image pointer if space is pressed
        if(image == &original_image){
          image = &modified_image1;
        }
        else if(image == &modified_image1){
